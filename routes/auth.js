...
router.post('/', async (req, res) => {
  ...
  if (user.isFrozen) return res.status(400).send({ message: lockedMessage })
  ...
  if (!validPassword) {
    ...
    if (!user.isFrozen) {
      ...
      return res.status(400).send({ message: lockedMessage })
    }
  }

  const token = user.generateAuthToken()
  // production environment,  secure:true TODO
  ...
  // res.cookie(config.get('cookieName'), token);
  //  token property is only needed in case frontend cannot read cookies
  const pickedFields = _.pick(user, [
    '_id',
    'name',
    'email',
    'isAdmin',
  ...
  res.send({
    data: { ...pickedFields, tokenFromServer: token },
...
router.post('/send-reset-email', async (req, res) => {
  ...
  let url = `${frontUrl}/reset-password?token=${token}`
  try {
    ...
    tokenRecord.expireDate = new Date(
      currentDate.getTime() + expiredMinutes * 60 * 1000
    )
    await tokenRecord.save()
    await newResetPassword(user.name, url, email)
    return res.status(200).send({
      message: 'A password reset email has been sent to your email addresss.',
    })
  } catch (error) {
...
router.post('/reset-password', async (req, res) => {
  ...
  if (!tokenRecord) return res.status(400).send({ message: 'Invalid Token.' })

  let now = new Date()
  if (now > tokenRecord.expireDate)
